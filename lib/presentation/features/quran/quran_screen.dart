  ...
  Widget build(BuildContext context) {
    return BlocListener<QuranBloc, QuranState>(
      listener: (context, state) {
        if (state is QuranNotFound) {
          SnackbarHelper.show(
            context,
            state.message.tr,
            type: SnackBarType.warning,
          );
        } else if (state is QuranBookmarkedToDetail) {
          context.push(AppRouterConst.quran.detail(state.quranId));
        } else if (state is QuranLastReadToDetail) {
          context.push(AppRouterConst.quran.detail(state.quranId));
        }
      },
      child:  Container(
                        ...
                        Row(
                          children: [
                            ActionCard(
                              ...
                              color: AppColors.primary,
                              onTap: () {
                                context.read<QuranBloc>().add(const GetLastReadSurahId());
                              },
                            ),
                            SizedBox(width: 12),
                            ActionCard(
                              ...
                              title: 'bookmarks'.tr,
                              onTap: () {
                                context.read<QuranBloc>().add(const GetBookmarkedSurahId());
                              },
                            ),
                          ],
                        ),
